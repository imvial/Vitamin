...
class Storage():
    ...
    def __init__(self, st_name, path=None, extensions=[], conversions=None, fakepath=None):
    ...
    def __getattribute__(self, name):
        ...
        if (not name in conversions) and (name in files):
            return default.go(name, self.files[name], self._fakepath)
        else:
...
class StorageSystem(object):
    ...
    def __init__(self, site_config):
        ...
        for name, params in self.FOLDERS.items():
            ...
            self.add_storage(Storage(name, path, extensions,
